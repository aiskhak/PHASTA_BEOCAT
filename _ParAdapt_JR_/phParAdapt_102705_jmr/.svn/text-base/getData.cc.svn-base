...
void getX(pMesh mesh, double **x)
{
  ...
  int dof=0;
  void *temp=0;
  while(vertex = (pVertex)M_nextVertex(mesh,&temp))
...
void getConnectivity(pMesh mesh,pPList bdry, int ***ien, int ***ienb, globalInfo* info)
{
  ...
  while (region = RIter_next(rIter)){
    ...
    poly = EN_dataI((pEntity)region,"POLY");
    ...
    for (i=0; i < nen; i++)
     ien[blockid][iel[blockid]][i] = EN_dataI((pEntity) Rvertices[i], "MYCT");
    ...
    if(info->edgeson) {
      ...
      for (i=0; i < nedges; i++){
        ...
        for (j=0; j < nem; j++){
          ...
          ien[blockid][iel[blockid]][ldof++] =
                                EN_dataI((pEntity) Redges[i], "MYCT") + j;
          if((esign == 0) &&(msign ==0))
      ...
      for(i =0; i < nfaces; i++){
        ...
        if (nvf == 3){
          ...
          for (ip = 3; ip <= PolyOrd; ip++){
            ...
            for (is=0; is < nsh; is++){
              ien[blockid][iel[blockid]][ldof++] =
                                EN_dataI((pEntity) Rfaces[i], "MYCT") + j++;
              if ((fsign == 0) && (msign == 0))
                ien[blockid][iel[blockid]][ldof-1] *= -1;
            }
          }
          ...
          for(ip = 4; ip <= PolyOrd; ip++) {
            ...
            for(is =0; is< nsh; is ++) {
              ien[blockid][iel[blockid]][ldof++] =
                                EN_dataI((pEntity) Rfaces[0], "MYCT") + j++;
    ...
    if(info->regnson){
      nrm = EN_dataI((pEntity) region, "NDOF");
      if (nrm > 0)
        for (j=0; j < nrm; j++)
          ien[blockid][iel[blockid]][ldof++] =
                        EN_dataI((pEntity) region, "MYCT") + j;
    }
  ...
  void* temp = 0;
  while (face =(MFace *) PList_next(bdry,&temp)){
    ...
    for (i=0; i < nen; i++){
      ienb[blockid][ielb[blockid]][i] =
              EN_dataI((pEntity) Rvertices[i], "MYCT");
    }
    ...
    if(info->edgeson) {
      ...
      for (i=0; i < nedges; i++){
          ...
          for (j=0; j < nem; j++){
            msign = h_modeSign(1,j+2);
            ienb[blockid][ielb[blockid]][ldof++] =
                 EN_dataI((pEntity) Redges[i], "MYCT") + j;
            if((esign == 0) &&(msign ==0))
