...
#ifdef PARALLEL
extern void writeEnsaFiles(pParMesh pmesh, globalInfo *info, pPList bdry)
#else
...
#endif
{
  ...
  */
...
#ifdef PARALLEL
  writeNCorp(pmesh, info, &ifath, &nsons);
#else
  ...
  if(lStart){ //lin-2-quad
    ...
    qTot = (double *)calloc(nv*(info->nshgTot), sizeof(double));
    ...
    //
    restart(qTot, info->nshgTot, nshgReqst, rRead, &lstep, mesh);
  }
      ...
      nshgReqst = info->nshg;
      qTot = (double *)calloc(nv*(info->nshg), sizeof(double));

      for (i=0; i< nv*(info->nshg); i++){
        qTot[i]=0;
      }
      // greps/reads solution from adaptor/file: implemented in restart.cc
      restart(qTot, info->nshg, nshgReqst, rRead, &lstep, mesh);
      if(PMU_rank() == 0){
  ...
  earr.write(mesh, info, ePar, zscale, ifath, nsons);
