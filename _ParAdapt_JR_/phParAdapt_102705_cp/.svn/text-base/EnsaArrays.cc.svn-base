  ...
  : _info(info)
{
  ...
  num = info->nshg;
  q = new double*[num];
  for (j=0; j < num; j++) q[j] = new double[numVars];
...
void EnsaArrays::write(pMesh mesh, globalInfo *info, const EnsaParameters &par,
                       double zscale[3], int *ifath, int *nsons)
{
  int i, j, k, ind, *ienf, *iBCBf, *ienbf;
  char fname[30], proc[5];
  double *xf, *BCf, *BCBf, *qf;
  FILE *fgmasc;
  FILE *fbcasc;
  FILE *frsasc;
...
//      delete[] BCBf;
  ...
  if(phastaIO==1){
      ...
      sprintf(dirname,"%d-procs_case/",PMU_size());
      ...
      sprintf(filename,"%sgeombc.dat.%d",dirname,PMU_rank()+1 );
      openfile_( filename, "write", &fgeom );
      
      bzero( (void*)filename, 255 );
      sprintf(filename,"%srestart.%d.%d",dirname, lstep, PMU_rank()+1 );
