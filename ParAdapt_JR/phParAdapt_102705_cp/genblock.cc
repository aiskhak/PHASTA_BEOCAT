...
void genblock(pMesh mesh, globalInfo *info, pPList bdry)
{
  ...
  NIblock = new int [12];
  NBblock = new int [12];
  RIblock = (blockKey*)malloc(12*sizeof(blockKey));
  RBblock = (blockKey*)malloc(12*sizeof(blockKey));
  Nshape = new int [12];
  NshapeB = new int [12];
  for(int j=0; j< 12; j++) {
    NIblock[j] =0;
  ...
  while (region = RIter_next(mrIter)){
    bKey.maxpoly = EN_dataI((pEntity)region,"POLY");
    bKey.nen = EN_dataI((pEntity)region,"RNEN");
