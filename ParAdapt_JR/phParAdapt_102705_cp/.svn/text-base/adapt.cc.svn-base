...
//     PM_setMigrId(pmesh, incorp);
  ...
  case 2 : { //anisotropic adaptation
    ...
    simAdapter = MSA_newPar(pmesh,1,1);

    // invoke solution transfer
    MSA_setCallback(simAdapter, phastaTransfer,9,NULL);// 8 for regions, 1 for vertices
...
//        SmoothHessians(mesh);      
  ...
  case 4 : { // isotropic adaptation (tag driven)
    ...
    simAdapter = MSA_newPar(pmesh,0,1);//
...
//    writeSmoothEIs(mesh);
    ...
    transformToScalarErrorVal(mesh,nvar);
  ...
  break;
//   case 8: {
//     cout<<"\nStrategy choosen to read size-field info. from file"<<endl;

//     char sfFilename[256];
//     sprintf(sfFilename,"sizefield.%i.%i.bb",lstep,PMU_gid(PMU_rank(),0)+1);

//     simAdapter = MSA_newPar(pmesh,1,1);

//     setSizeFieldFromFile(mesh,simAdapter,sfFilename);
...
//   break;
  ...
  if(masterProcWgt>0.) {
    ...
    if(tnp>1) {
      ...
      PartitionOpts_setTotNParts(popts,tnp);
      PartitionOpts_setPartWtEqual(popts);
      
      PM_partition(pmesh,popts);
  ...
  for(  int i =0 ; i < PM_numParts(pmesh); i++ ) {
    gproc = PMU_totPartFromGid( PM_totNParts(pmesh), PMU_gid( PMU_rank(), i )) ;
    mesh = PM_mesh(pmesh, i);
    sprintf(stringName, "refined_p.%d.sms",gproc+1);
    M_writeSMS( mesh, stringName, version);
  }
...
void printTimeStatsToFile(int strategy) 
{
  ...
  fprintf(tFile,"MSA_newPar() & MSA_setCallBack() : %6.2f\n",
