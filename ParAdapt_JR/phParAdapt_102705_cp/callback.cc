...
void
phastaTransfer( MeshModType mtype, pMeshChanges mco, void *userData)  {
...
//    printf("\n entering callback fct\n");
    ...
    pPList freshEnts = MCO_getCreatedEntities(mco); 
    pPList oldEnts  = MCO_getDeletedEntities(mco);
        ...
        for( i =0; i < PList_size(newVertices  ) ; i++ ){
            ...
            if(! PList_inList(verticesO,v)){
                ...
                if(! EN_getDataPtr( ( pEntity )v, phasta_solution, (void
                    ...
                    // provide  memory for new solution
		  // solution= (double *)malloc(sizeof(double)*numVars );
...
int delDblArray(pAttachableData ad, pAttachDataId id, int cb, void** data, void* ent) {
...
int delDbl(pAttachableData ad, pAttachDataId id, int cb, void** data, void* ent) {
