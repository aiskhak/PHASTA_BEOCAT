...
c      call readdatablock (irstin, fnamer, qstat, isize, 'double',iotype)
...
c      OPEN(867,FILE='stats.out-ascii-procstats')
c      WRITE(867,*)" machine  nshg  lstep  "
c      WRITE(867,*) machine, nshg, lstep
c      DO I=1, nshg
c         WRITE(867,"(5(F24.8))")(qstat(I,J),J=1,19)
c      ENDDO
      
c      CLOSE(867)

     
c      call closefile( irstin, "read" )

c      do i=1,19
c         do j=1,nsd
c            qstat_new(mapb(j)+i*nsd,:) = qstat(j+i*nsd,:)
c            qstat_new(j+i*nsd,:) = qstat(mapb(j)+i*nsd,:)
c            qstat_new(j,:) = qstat(j+i*nsd,:)
c         enddo
c      enddo

c
c  Add this guy to the accumlated vector (to allow multiple
c  averaged segments to be combined)
c
c      wghta=wghta+wght
      qstr=qstr+qstat
...
c
      do i = 1, numnp
         qstr(i,:) = qstat(invmap(i),:)/real(nsteps,8)
      enddo
c
...
c
     ...
     &     itmpo,itmp
      ...
      do i = 1, Nx
	    ...
	    ind2 =  (i-1)*Ny + j
      write (iout, 111) xyz(ind,1),xyz(ind,2),(qsta(ind2,k), k=2,4)
            write (iout2,112) 
     &		xyz(ind,1),xyz(ind,2),(qsta(ind2,k), k=7,12)
            write (iout3,112) 
     &		xyz(ind,1),xyz(ind,2),(qsta(ind2,k), k=14,19)
            write (iout4,110) 
     &		xyz(ind,1),xyz(ind,2),qsta(ind2,1)
            write (iout5,110) 
     &		xyz(ind,1),xyz(ind,2),qsta(ind2,6)
         enddo
      enddo
