...
int geometry_and_connectivity(int* array, double* &xglobal, int* &ien, 
    ...
    iarray[0] = -1;
    readheader_(&igeom,"number of nodes in the mesh",(void*)iarray,&ione,"integer",iotype); 
    numnp = iarray[0];
    if(iarray[0] == -1){
    ...
    iarray[0] = -1;
    readheader_(&igeom,"number of spatial dimensions",(void*)iarray,&ione,"integer",iotype); 
    nsdread = iarray[0];
    if(iarray[0] == -1){
    ...
    iarray[0] = -1 ;
    readheader_( &igeom, "edge mode mapping to sms",(void*)iarray, &itwo, "integer", iotype);
    if ( iarray[0] > 0 ) {
        esize = iarray[0]; emodes = iarray[1];
        sms2edge = new int [ esize ];
        readdatablock_( &igeom, "edge mode mapping to sms", (void*)sms2edge, &esize, "integer", iotype );
    }
                                                 
    iarray[0] = -1 ;
    readheader_( &igeom, "face mode mapping to sms",(void*)iarray, &itwo, "integer", iotype);
    if ( iarray[0] > 0 ) {
        fsize = iarray[0]; fmodes = iarray[1];
        sms2face = new int [ fsize ];
        readdatablock_( &igeom, "face mode mapping to sms", (void*)sms2face, &fsize, "integer", iotype );
    }
  ...
  /* scanning geom.dat.<procnum> to add up neltot and to determine maxnshg */
	...
	nelblk=iarray[0];
        ...
        iarray[0] = -1;
	readheader_(&igeom,"co-ordinates",(void*)iarray,&itwo,"double",iotype);
        np=iarray[0];
        nsd=iarray[1];
	if(iarray[0] == -1){
        ...
        if ( numprocs > 1 ) {
            iarray[0] = -1;
	    readheader_(&igeom,"mode number map from partition to global",(void*)iarray,&ione,"integer",iotype);
            nshgl=iarray[0];
            if(iarray[0] == -1){
	    ...
	    ncorp2d[i] = (int * )malloc(sizeof(int)*nshgl);
            readdatablock_(&igeom,"mode number map from partition to global",
        ...
        for(k=0; k< nelblk; k++){
            ...
            iarray[0] = -1;
	    readheader_(&igeom,"connectivity interior",(void*)iarray,&iseven,"integer",iotype);
            neltp  =iarray[0];
            nenl   =iarray[1];
            ipordl =iarray[2];
            nshl   =iarray[3];
