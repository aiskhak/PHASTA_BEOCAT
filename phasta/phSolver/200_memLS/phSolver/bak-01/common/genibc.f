...
c
	   ...
	   integer Mtype, Mnum
        ...
        real*8   rlfrad(maxp)                                     !Matt Thomas
        integer  Num_reliefs                                      !Matt Thomas
...
c       if(numpbc.eq.0) return  ! sometimes there are no BC's on a partition !Farhad. I commented this line out
        ...
        !Matt Thomas Circular Relief areas
        inquire(file='pressure_relief.dat',exist=exts)
        if((exts).and.(myrank.eq.master))then
          write(*,*)'Pressure_relief file is available'
        end if
        if(exts)then
         open(unit=89, file='pressure_relief.dat',status='old')
          do m=1,3
           read(89,*)
          end do
          read(89,*)Num_reliefs   !number of relief areas
          do i=1,Num_reliefs  
           read(89,*)rlfrad(i), rx(i), ry(i), rz(i) !relief radius, relief area center coords
          end do
          close(89)

          do n = 1, nshg
           do i=1, Num_reliefs
            RadiusF=sqrt((x(n,1)-rx(i))**2.0d0+(x(n,3)-rz(i))**2.0d0) !This only works for xz-plane reliefs
             if(abs((x(n,2)-ry(i))**2.0d0) .le. 1.0d-18) then         !This only works for xz-plane reliefs
              if(RadiusF .le. rlfrad(i)) then
               iBC(n)=4
              end if
             end if
           end do !n
...
!	 write(*,*) 'Mnum = ', Mnum
	   ...
	   do m = 1, Mz
		read(89,*) rz(m)
           ...
           read(89,*) Msize, Mnum ! Type: 3 - triangle; Size is height, Mnum is number of items
           do m = 1, Mnum
                read(89,*) rx(m), ry(m), rz(m)
           end do
	 end if      
      ...
      if (exts) then  ! Igor
!temporarily       do n = 1, nshg
!removed       RadiusF = sqrt((x(n,3)-0.0125)**2.0 +    !!!!HARDCODED COORDINATES AND RADIUS!!!
!in order     1  (x(n,2)-0.0)**2.0 + (x(n,1)-0.00625)**2.0)    !  Sphere shaped - channel flow turbulence
!to do         if (RadiusF .lt. 0.0035) then    ! Igor, sphere shaped for channel flow turbulence
!ramp at            iBC(n)=56                     !HARDCODED COORDINATES AND RADIUS!!!!!
!domain         end if
!exit       end do
...
!		  write(*,*) 'Fixed point N, x, y, z: ', n, x(n,1), x(n,2), x(n,3)
        ...
        enddo  ! m
       enddo   ! n
         elseif (Mtype.eq.3) then   ! triangluar roughness done by Matt Thomas
        do n = 1, nshg
           do m = 1, Mnum
                qorny = ry(m)+0.5*Msize
                qornx = rx(m)+0.5*Msize
                tri_box_slop = (qorny - ry(m))/(qornx - rx(m))
                t_bx_intrcpt = ry(m) - tri_box_slop*rx(m)
                up_tri_bndy = tri_box_slop*x(n,1) + t_bx_intrcpt
                xleft = x(n,1) - (rx(m)-0.5E0*Msize)
                xright = (rx(m)+0.5E0*Msize) - x(n,1)
                yleft = x(n,2) - (ry(m)-0.5E0*Msize)
                yright = (up_tri_bndy) - x(n,2)
            if (xleft.gt.0.and.xright.gt.0
     1     .and.yleft.gt.0.and.yright.gt.0)then
                   iBC(n)=56
                endif
           enddo  ! m
       enddo   ! n
