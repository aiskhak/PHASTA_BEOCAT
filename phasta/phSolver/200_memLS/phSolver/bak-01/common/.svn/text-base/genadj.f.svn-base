      subroutine genadj (colm,         rowp, icnt, iper )
c     
      use pointer_data
c     
      include "common.h"
c     
      integer rowp(nshg*nnz),         colm(nshg+1), iper(nshg)
...
c                                   also change overflow check in asadj TWICE
      ...
      adjcnt=0
      if (myrank.eq.master) write(*,*) ' point 1 '
...
! Igor, December 2012: Hack into here to account for the potential
! non-zero if i = iper(j). Note, non-zero is at (j,i)
...
! Igor:
!        if (iper(i).ne.i) then
!         adjcnt(i) = adjcnt(i) + 1  ! Igor!
!         row_fill_list(i,adjcnt(i)) = iper(i)  ! Igor!
!         write(*,*) 'iper is hacked in genadj!: ', i, iper(i)
!        end if
