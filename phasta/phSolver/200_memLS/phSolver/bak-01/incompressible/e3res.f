...
c
     ...
     &          dVolume(npro), elem_shell_num  ! bubble boiling
...
c  10   format('rho = ', I4, 500F5.2)
      ...
      enddo
        ...
        call Bubvolgenera(dVolume, elem_shell_num)
        endif
c      write(*,*)'dVolume, elem_shell_num', dVolume, elem_shell_num
c
c
...
c        dVolume(i) =2*dVolume(i) ! artificially makes it grow faster       
        if ((sclr_ls(i).GT.-2.0E0*epsilon_lsd_tmp).and.
     &    (sclr_ls(i).LT.-1.0E0*epsilon_lsd_tmp)) then
     ...
     &              * ( shg(i,aa,1) * uBar(i,1)
     &                + shg(i,aa,2) * uBar(i,2)
     &                + shg(i,aa,3) * uBar(i,3) )+ 1e-10
c     &                + shpfun(i,aa)*(dVolume(i)/elem_shell_num/nshl)
         enddo
c        write(*,*)'rl(i,aa,4)', rl(i,aa,4)
         else
...
c
     ...
     &                + shg(i,aa,3) * uBar(i,3) )
       ...
       end do ! i = 1, npro - loop over elements at local level
