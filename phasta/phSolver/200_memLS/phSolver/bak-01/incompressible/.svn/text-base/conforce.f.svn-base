...
!----------------------------------------------------------------------
        ...
        ncoalEvent      = 0
        do ib = 1, i_num_bubbles-1
           if(avg_info(ib,4).gt.0.0d0) then
              do jb = ib+1, i_num_bubbles
                 if(avg_info(jb,4).gt.0.0d0) then
                    bubSurfDist = sqrt(
     &                        (avg_info(ib,1)-avg_info(jb,1))**2.0d0 
     &                       +(avg_info(ib,2)-avg_info(jb,2))**2.0d0
     &                       +(avg_info(ib,3)-avg_info(jb,3))**2.0d0
     &                       ) - avg_info(ib,4) - avg_info(jb,4)
                    if(bubSurfDist.le.8.0d0*epsilonBT)
     ...
     &                       +(avg_info(ib,3)-avg_info(jb,3))**2.0d0
     &                       ) - avg_info(ib,4) - avg_info(jb,4)
                    if(bubSurfDist.le.8.0d0*epsilonBT) then
                       icoalEvent = icoalEvent + 1
                       coalCenter(icoalEvent,1) =
     &                 (avg_info(ib,1)+avg_info(jb,1))/2.0d0
                       coalCenter(icoalEvent,2) =
     &                 (avg_info(ib,2)+avg_info(jb,2))/2.0d0
                       coalCenter(icoalEvent,3) =
     &                 (avg_info(ib,3)+avg_info(jb,3))/2.0d0
                    endif
                 endif
              enddo
...
!----------------------------------------------------------------------
     ...
     &                 )
           ...
           if(ctrlRange.le.5.0d0*epsilonBT) then
              Wetmp = CoalInvSigma
!              write(*,*)'CoalInvSigma =', CoalInvSigma
              exit
           endif
        enddo
