...
c!  bub_info(npro,11)            : bubble information collection array
...
c
	...
	real*8    A11, B11   ! Wall repellant force now depends upon the Weber number, Igor, April 2010
     ...
     &            appvolume(ibksiz,nenl,coalest) ! Matt Talley: Coalesc. Control
        ...
        real*8    fit
        dimension Tempb(npro), gytemp(npro,5), shell_elem(npro,2) ! Mengnan: temperature
        real*8    Tempb, gytemp
        real*8    shell_elem, num
c
...
!        write(*,*)"I'm here"
        call Bubheatflux(yl, shpfun,  shg, Tempb,  gytemp,  shell_elem)
        else
        Tempb=zero
        gytemp=zero
        shell_elem=zero
