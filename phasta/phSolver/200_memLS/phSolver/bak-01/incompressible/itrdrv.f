...
!  Mass conservarion Epsilon_lsd adjustment:
	real*8 vf, vf0, eps_new, mass_error

!c.... For matt's control force algorithm
        integer ierror, numoldyhistind

        real*8 sumxcf,sumxcf_o
        real*8 sumycf,sumycf_o
        real*8 sumzcf,sumzcf_o
        real*8 sumycf_old
!... for simulation steering...
        integer inumstart
        character*10 inpfname

        integer N_Lsc  !to compute how many times level set meets
                       !convergence criteria

!... Some variables are kept in itrdrv.f when the major part of control
!... force is moved in the subroutines
...
!       The domain center coordinates will be used in bubble center adjustment
...
!           write(*,*) 'lstep in itrdrv =', lstep
           if(myrank.eq.master) then
              call OpenBubFiles(i_num_bubbles, C_int_adjust)
              write(*,*)
...
!        do istp = 1, nstp
     ...
     &        allocate( avg_info(i_num_bubbles,21) )
...
!----------------------------------------------------------------------
        vf_now = vf
!       Target void fraction has to be positive to enable the following
!       algorithm. 
        IF(iFT.eq.0.and.vf_target.gt.0.0d0) THEN
...
!       print out the results and save them in files
...
!       broadcast the bubbles centers and volumes for the marker field updating
!       algorithm
           if(numpe > 1) then
...
!           if(myrank.eq.1) write(*,*) 'bub_cent =', bub_cent
