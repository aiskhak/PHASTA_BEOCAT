...
!     &                     /(datmat(1,1,1)*datmat(1,3,1)*0.3566d0)
     &                    /(datmat(1,1,1)*datmat(1,3,1))
              enddo
!             do iblk = 1, nelblb
!                if(myrank.eq.master)write(*,*)lcblkb(1,iblk)
!             enddo
!                 if(myrank.eq.master)write(*,*)nsurf
!               if(flux(iel).ne.0.0d0)write(*,*)'flux',flux(iel)
              else
!              flux(iel) = zero
              do n = 1, nshlb
                 nodlcl = lnode(n)
