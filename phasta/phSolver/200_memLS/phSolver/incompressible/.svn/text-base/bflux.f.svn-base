...
!          do n = 1, numflx
!             k = nodflx(n)
!             write (iflux,2000) k,invflx(k),flxLHS(k,1),(x(k,i),i=1,3), 
!      &           (flxnrm(k,i),  i=1,3),
!      &           (flxres(k,i),  i=1,3)
!          enddo
!          close (iflux)
...
c         iarray(2) = ndof
c         iarray(3) = lstep
c         call writeheader(irstin, fieldbflux,iarray, nitems, isize, 
c     &        'double', iotype )
    
cc         fieldbflux = 'boundary flux'        
c         nitems = nshg*ndof
c         call writedatablock(irstin, fieldbflux,rtmp, nitems, 
c     &        'double', iotype)
c        
c         call closefile( irstin, "append" )
cc         call Write_boundaryflux(myrank,lstep,nshg,ndof,rtmp(:,1:ndof))

	call write_field(myrank,'a','boundary flux', 
     1  13, rtmp, 'd', nshg, ndof, lstep)

c     wallss vectors into the restart file(s)
         if( iowflux .eq. 1) then
c            call openfile(  fname2,  'append?', irstin )
            
c            fieldwss = 'wall shear stresses'
c            isize = nshg*ndof
...
c            iarray(2) = ndof
c            iarray(3) = lstep
c            call writeheader(irstin, fieldwss, iarray, nitems, isize, 
c     &           'double', iotype )
         
cc            fieldwss = 'wall shear stresses'        
c            nitems = nshg*ndof
cc     wall shear stresses vectors  are in wallssVec
c            call writedatablock(irstin, fieldwss, wallssVec, nitems, 
c     &           'double', iotype)
            
c            call closefile( irstin, "append" )         

        call write_field(myrank,'a','wall shear stresses', 
     1   19, wallssVec, 'd', nshg, ndof, lstep)
