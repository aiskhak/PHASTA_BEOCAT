...
c used locally
     ...
     &        u1_tmp_temp(npro), u2_tmp_temp(npro),
     &        u3_tmp_temp(npro)
...
c.... Adjust the bubble interface velocity to expand or contract
c.... the bubble to simulate evaporation or condensation
c            if (bubboil.eq.1.or.bubgrow.eq.1) then
c               do i = 1,npro
c                  u1_tmp_temp(i) = u1(i) + 1.0d-3 * (Temp(i) - 1.0d2)
c     &                           * gradS(i,1)
c                  u2_tmp_temp(i) = u2(i) + 1.0d-3 * (Temp(i) - 1.0d2)
c     &                           * gradS(i,2)
c                  u3_tmp_temp(i) = u3(i) + 1.0d-3 * (Temp(i) - 1.0d2)
c     &                           * gradS(i,3)
c               enddo
c
c               u1(:) = u1_tmp_temp(:)
...
c... associated with level set method 
            do i = 1,npro
                  ...
                  u1_tmp(i) = u1(i) + C_int_adjust * gradS(i,1) ! KEEP HERE
...
! Lets assume that all the scalars actually convective: e.g. travel with
! fluid velocity (Igor, April 2013)
