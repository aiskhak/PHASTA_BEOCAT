...
c
	...
	real*8	lhsK(9,nnz_tot),	lhsP(4,nnz_tot)
        ...
        real*8 cfl(nshg), CFLfl_maxtmp, cflbuint(nshg), CFLbuint_maxtmp
        integer icflhits(nshg), icflbuinthits(nshg)
!Mengnan Evaporation & condensation
        real*8 numshell_temp2, bubble_vol_temp
...
c
        ...
        cfl = zero
        icflhits = 0
        cflbuint = zero !Initialize for cfl at bubble int Matt T.
        icflbuinthits = 0
c
...
c!... arrays used in bubble tracking advanced analysis
        ...
        allocate ( procs_dataset(i_num_bubbles, 24) )
...
c!... assemble local element volume matrix and bubble information array
          if(iBT .eq. 1)  allocate( bub_info(npro,24) )
          if(iCForz.eq.1) allocate( cf_var(npro,15)   )

          allocate(elemvol_local(npro))
          elemvol_local(:)=elemvol_global(iblk,iel:iel+npro-1)
...
c
     ...
     &                 xKebe,     xGoC,     rerr,
     &                 cfl,       icflhits,    
     &                 elemvol_local, cflbuint, icflbuinthits, 
...
c!  the average CFL number.
       cfl = cfl / icflhits
       cflbuint = cflbuint / icflbuinthits
...
c
        ...
        do i = 1, nshg
          ...
          if (cflbuint(i).gt.CFLbuint_max) then
             CFLbuint_max = cflbuint(i)
          endif
        enddo !i
...
c!     &   , MPI_MAX, MPI_SUM
     ...
     &       MPI_DOUBLE_PRECISION,MPI_MAX, MPI_COMM_WORLD,ierr)
           call MPI_ALLREDUCE (CFLbuint_max, CFLbuint_maxtmp, 1,
     &       MPI_DOUBLE_PRECISION,MPI_MAX, MPI_COMM_WORLD,ierr)  
c!	  write(*,*) 'myrank, ierr = ', myrank, ierr 
c!           if (myrank .eq. master) write(*,*) 'MPI reduce is done'
        else
           CFLfl_maxtmp = CFLfl_max
           CFLbuint_maxtmp = CFLbuint_max
        endif

        CFLfl_max = CFLfl_maxtmp
        CFLbuint_max = CFLbuint_maxtmp !Reassign the value Matt T.
...
c
c... routine to add resistance of current step to faceRef array
c
