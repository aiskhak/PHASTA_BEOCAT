...
c------------------------------------------------------------------------
     ...
     &          acl(npro,nshl,ndof),
     &          rlsli(npro,6)
      dimension bml(npro,nshl,1)  ! store the maker field for bubble tracking
                                  ! Mengnan
...
!        if(dVolume(j).ne.0.0d0)then
!         write(*,*)'bubgrow',bubgrow
!         write(*,*)'bubboil',bubboil
!        endif
...
c        bubsource=zero            
     ...
     &                 elem_local_size(lcblk(1,iblk)+i-1)
        ...
        if ((bubgrow.eq.1.0.or. bubboil.eq.1.0).and.
     &      (shell_num(i).ge.2.0d0))then
  
        ! Remove phase-change volume from liquid phase
                if ((sclr_ls(i).GT.1.0E0*epsilonBT).and.
     &            (sclr_ls(i).LT.2.0E0*epsilonBT)) then
...
c         write(*,*)'liquid phase', dVolume(i)/shell_num(i)
              

                elseif ((sclr_ls(i).GT.-2.0E0*epsilon_ls_tmp).and.
     &              (sclr_ls(i).LT.-1.0E0*epsilon_ls_tmp)) then

!         if(shell_num(i).lt.1.0)
!     &      write(*,*)shell_num(i)
...
c     &                 *(dVolume(i)/shell_num(i)/nshl)
                 enddo
c                 write(*,*)'gas phase', dVolume(i)/shell_num(i)
           
c          shell_num_old(i)=shell_num_new(i)
c       if(dVolume(i).ne.0.0d0)write(*,*)
c     &                 dVolume(i),shell_num(i)
       
        
! Remove phase-change volume from liquid phase
!         elseif ((sclr_ls(i).GT.1.0E0*epsilonBT).and.
!     &    (sclr_ls(i).LT.2.0E0*epsilonBT)) then
!
!           do aa = 1, nshl
!               rl(i,aa,4) = rl(i,aa,4) + WdetJ(i)
...
!           enddo
!
!         write(*,*)'liquid phase', dVolume(i)/shell_num(i)
               else
...
!        bubsource_sum=0.0
!        do k = 1, npro
!        bubsource_sum = bubsource_sum + bubsource(k)
!        enddo
        endif  ! boiling flag
       enddo ! i = 1, npro - loop over elements at local level
!      if(myrank.eq.master)then
!         write(*,*)'shell_num_old(1)',shell_num_old(1)
!      endif
!       if(bubsource_sum.ne.0.0d0)then       
!        write(1991,*)lstep+1,bubsource_sum
!      endif
      do aa = 1,nshl
c
c.... momentum
...
c
      ...
      rcp  = one ! rho * cp
c      rcp = one     
c............... Heat convection Term ...................Mengnan Li
         rNa = rcp*(tmps*Sdot + uMod(:,1) * gradS(:,1)
     &                         + uMod(:,2) * gradS(:,2)
     &                         + uMod(:,3) * gradS(:,3))  
     &        - src

c      rcp  = datmat(1,4,1)/(datmat(1,1,1)*datmat(1,3,1)) ! rho * cp   
c      rcp = one
...
!      if(myrank.eq.master) write(*,*)'diffus',diffus
c
