     ...
     &                 rerrl,   rlsl,    cfll,    evl,
     &                 xwl,     ywl,     zwl,     
     &                 elemvol_local,    cflbuintl,
     &                 xarray,  yarray,  zarray,  bubradius,
     &                 bubradius2, coordtag, bml)
c                                                                      
c----------------------------------------------------------------------
c
...
c  cflbuintl (npro,nshl)         : CFL of the element around the bubble interface
c
...
c
     ...
     &            zwl(npro,nenl),         cflbuintl(npro,nshl)
c
...
c
        real*8 cfll_loc(npro), cflbuintl_loc(npro)
        dimension Sclr(npro)
...
c
     ...
     &                cfll_loc)
        ...
        if (buintcfl.eq.1) then
           do i = 1, npro ! Find the clf local at the bubble interface
              if (abs(Sclr(i)).le.3.0d0*epsilon_ls_tmp) then !Matt Talley
                 cflbuintl_loc(i) = cfll_loc(i)
              else
                 cflbuintl_loc(i) = 0.0d0
              endif
           enddo
        endif
        ...
        if (buintcfl.eq.1) then
           do k = 1, nshl
             cflbuintl(:,k) = cflbuintl(:,k) + shpfun(:,k)*cflbuintl_loc
           enddo
        endif
