...
!----------------------------------------------------------------------
        ...
        IF(nBubbleLTS.eq.i_num_bubbles) THEN
c = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
!       This regular part
        C_int_adjust = 0.0d0
        write(*,*)
...
!----------------------------------
     ...
     &                  .and. irstart+1.ne.inistep) then 
          ...
          allocate (bubmatx(irstart+1-inistep,24))
...
!       Reading the old bubble information into temporary matrix
     ...
     &          status="old", action="read", iostat=ierror)
               ...
               do il = 1, (irstart+1-inistep)
                  read(file_number,812) bubmatx(il,1:24)
               enddo
               close(file_number) 
...
!       Writing the temporary matrix into newly created files
     ...
     &         action="write", iostat=ierror)
               ...
               do il = 1, (irstart+1-inistep)
                  write(file_number,812) bubmatx(il,1:24)
               enddo
               close(file_number)
...
c = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
!       The second part is used to open files when breakup event is
!       recognized
        ELSEIF(nBubbleLTS.lt.i_num_bubbles) THEN
        write(*,*) 'PHASTA opens new bubble files'
        call CountAlphaLines(nl_alpha,inistep)
        write(*,*) 'Number of timesteps recorded = ', nl_alpha

        allocate (bubmatx(1,24))
        bubmatx         = zero

        path1 = "../bubStat/bubble."
        do i=nBubbleLTS+1,i_num_bubbles
           write(path2,'(i4.4)') i
           fname_bub = trim(path1)//trim(path2)//".dat"
           file_number = 950 + i
!       Create new file with zero information filled for previous
!       time steps
           open(unit=file_number, file=fname_bub,
     &          status="replace", action="write", iostat=ierror)
           if(ierror /= 0) stop "Error reading bubble.*.dat"
           do il = 1, nl_alpha
              write(file_number,812) bubmatx(1,1:24)
           enddo
           close(file_number)
         end do     !i_num_bubbles
 ...
 812  format(ES14.7, 1x, ES14.7, 1x, ES14.7, 1x, ES14.7, 1x,
     ...
     &       ES14.7, 1x, ES14.7, 1x, ES14.7, 1x, ES14.7, 1x,
     &       ES14.7, 1x, F9.1,   1x, ES14.7, 1x, ES14.7, 1x,
     &       ES14.7, 1x, ES14.7, 1x, ES14.7, 1x, ES14.7, 1x,
     &       ES14.7, 1x, ES14.7, 1x, ES14.7, 1x, ES14.7, 1x)
...
!----------------------------------------------------------------------
        ...
        integer i_mrk, ib
        real*8  bub2w_temp, D_eq_temp1, D_eq_temp2
...
!        use bubboil_info  ! for boiling & condensation model
     ...
     &         bubble_tempG_temp, bubble_avgCA_temp, numshell_temp3
        allocate ( unive_dataset(i_num_bubbles, 24) )
...
!       a valid bubble
!  In addition, small information in the shell also will not be considered
           if(INT(unive_dataset(i,16)).lt.125.or.
     &        INT(unive_dataset(i,20)).lt.35.or.
     &        INT(unive_dataset(i,21)).lt.35) cycle
...
!       The local liquid shear around the bubble
     ...
     &     avg_info(i,10)   = (Shear(i,4) - Shear(i,2))/diffinY
           ...
           D_eq(i) = avg_info(i,4)*2.0d0
           bub2w(i,1:3) = avg_info(i,1:3)
           endif
...
!... Store temperature and temperature gradient around the bubble
          if(int(unive_dataset(i,18)).gt.1) then
...
!... Store the number of cell outside the bubble for liquid phase
           avg_info(i,24) = unive_dataset(i,24)
!... Store the Contact Angle information
        if(int(unive_dataset(i,22)).ne.0)then
           avg_info(i,22) = unive_dataset(i,23)/unive_dataset(i,22)
        endif
!           endif
        if(int(bubboil).eq.1.or.int(bubgrow).eq.1) then
           if (avg_info(i,19).ne.avg_info(i,19)) then
              write(*,*) 'NaN detected in TempG (adj to 5000.0): '
     &                   , avg_info(i,19)
              avg_info(i,19) = 5000.0
           end if 
           write(*,937) i, int(avg_info(i,20)),int(avg_info(i,21)),
     &               avg_info(i,18),avg_info(i,19), avg_info(i,22),
     &               avg_info(i,4)*2000.0
        endif
 
 937  format('No.', I3, 1x, 'Bubble; #el(out), #el(in)',
     &   I7, 1x, I7, 1x, '; temp,tempG', F8.2, 1x, F11.2, 
     &     '; CA = ', F6.1, '; D(mm): ', F5.2)

        if((int(CA_flag).eq.1).and.(int(bubboil).ne.1)) then
                write(*,*)'Current Contact Angle',
     &               avg_info(i,22)
        endif
!-------------------------------------------------------------------------------
...
! Check on the temp gradient:
        if (bubble_tempG_temp.ne.bubble_tempG_temp.
     &    or.bubble_tempG_temp.gt.1.0e20) bubble_tempG_temp = 1.0e4
        numshell_temp1    =  unive_dataset(i,20)
        numshell_temp2    =  unive_dataset(i,21)
        numshell_temp3    =  unive_dataset(i,24)
        bubble_avgCA_temp =  avg_info(i,22)
        endif
...
!          for number of element inside the bubble
          call MPI_Bcast(numshell_temp3,1,MPI_DOUBLE_PRECISION,
     &                       master,MPI_COMM_WORLD,ierr)
!          for number of element outside the bubble for liquid phase

          call MPI_Bcast(bubble_vol_temp,1,MPI_DOUBLE_PRECISION,
     &                   master,MPI_COMM_WORLD,ierr)
!          for each bubble vloume         
          call MPI_Bcast(bubble_avgCA_temp,1,MPI_DOUBLE_PRECISION,
     &                   master,MPI_COMM_WORLD,ierr)
!          call MPI_Bcast(bub2w,3,MPI_DOUBLE_PRECISION,
!     &                   master,MPI_COMM_WORLD,ierr)
!          call MPI_Bcast(bub2w(i,2),1,MPI_DOUBLE_PRECISION,
!     &                   master,MPI_COMM_WORLD,ierr)
!          call MPI_Bcast(bub2w(i,3),1,MPI_DOUBLE_PRECISION,
!     &                   master,MPI_COMM_WORLD,ierr)
!          call MPI_Bcast(D_eq,1,MPI_DOUBLE_PRECISION,
!     &                   master,MPI_COMM_WORLD,ierr)

          bubble_vol(i)  =  bubble_vol_temp
          bubble_tempG(i)=  bubble_tempG_temp
          numshell_out(i) =  numshell_temp1
          numshell_in(i) =  numshell_temp2
          numshell_out2(i) =  numshell_temp3
          bubble_avgCA  =  bubble_avgCA_temp
!        write(*,*)'bubble_avgCA',bubble_avgCA
       enddo
!       call MPI_BARRIER(MPI_COMM_WORLD,ierr)
       endif
       
!      if(myrank.eq.10) write(*,*) 'bubbleID', bubbleID(1), bubbleID(2)
!        write(*,*) 'numshell2 =', numshell(1,2)
...
!       If the node is in the near interface liquid shell
              elseif(yl(i,n,6).lt.deltS*epsilonBT.and.
     &               yl(i,n,6).gt.epsilonBT) then
...
!----------------------------------------------------------------------
     ...
     &            Sclr(npro)
        ...
        real*8, dimension(ibksiz):: CA_vector, CAnode
...
!       Contact Angle Calculation
          if(CA_flag.eq.1.0d0)then
              bub_info(i,22) = CAnode(i)
              bub_info(i,23) = CA_vector(i)
!         write(*,*) bub_info(i,22), bub_info(i,23)
         endif
...
!       22: The total volume of the outside bubble collection shell
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
              if (bubboil.eq.1.0.or.bubgrow.eq.1.0)then
                ...
                bub_info(i,19)= gytemp(i,5)
!                bub_info(i,22)= elemvol_local(i)
!                write(*,*)'bub_info(i,19)', bub_info(i,19)
               elseif(CA_flag.eq.1.0d0)then
                 bub_info(i,20)= 1.0d0
               endif
!              endif
!------------------------------------------------------------------------------
...
!                  bub_info(i,20)= 1.0d0 ! count element in the shell
!                 endif

                 if(Sclr(i).ge.-2.0d0*epsilon_lst_tmp
     &           .and.Sclr(i).le.-1.0d0*epsilon_lst_tmp)then ! inside
                                                
                  bub_info(i,21)= 1.0d0 ! count element in the shell
                 endif 
                 
!                 if (lstep.eq.52)then  
!                 if (bub_info(i,19).ne.0.0d0)then
!               if(bub_info(i,21).ne.0.0d0) 
!                   write(*,*)'bub_info(i,19)', bub_info(i,19)
!                 endif
!                endif
...
!     &                 breakupSeeder(ib,:)
           ...
           bub2w_temp = YLEN*0.5d0-abs(Ymid-bub2w(ib,2))
           D_eq_temp1 = D_eq(ib)*breakup2w    !0.64d0
           D_eq_temp2 = D_eq(ib)*(breakup2w + 1.5d0)
!           write(*,*)breakup2w
           if((mod(lstep,10).eq.0).and.
     &      (bub2w_temp.gt.D_eq_temp1).and.(bub2w_temp.lt.D_eq_temp2)) 
     &       iseeding     = 1
           write(*,*)bub2w_temp,D_eq_temp1
...
!----------------------------------------------------------------------
        ...
        real*8    y(nshg,ndof)
        real*8    Ratio, bub2w_temp, D_eq_temp
        dimension banma(nshg,1)
        integer   i, ib, i_mrk
...
!           bub2w = avg_info(ib,2)
!           D_eq = avg_info(ib,4)*2.0d0*0.6d0
!           write(*,*) breaksclr
           ...
           if(iactionFlag.eq.1) then
              ...
              if(myrank.ne.master .and. myrank.eq.irank) then 
                 do i = 1, nshg
                 i_mrk = int(banma(i,1)) 
                 if(y(i,6).lt.breaksclr.and. i_mrk.eq.ib) then
...
!               if(Ratio.lt.0.1d0 .or. Ratio.gt.0.995d0 .or.
!     &         (bub2w(ib,2).lt.(D_eq(ib)*0.64))) then
!                if(bub2w(ib,2).gt.0.0d0) 
                 bub2w_temp = YLEN*0.5d0-abs(Ymid-bub2w(ib,2))
                 D_eq_temp = D_eq(ib)*breakup2w    !0.64d0

                 D_eq_temp = D_eq(ib)*breakup2w    !0.64d0
!                if (bub2w_temp.lt.D_eq_temp)then
                if((Ratio.gt.0.01d0 .and. Ratio.lt.0.995d0))then !.and.
!     &          (bub2w_temp.gt.(D_eq_temp))) then
!       Modify upper limit 0.9 and lower limit 0.1 for boiling break-up  -MN 
!       case 1: fake trigger and no breakup is happening
!                  do i = 1, nshg
!                     if(int(banma(i,1)).eq.iauxBubID) 
!     &               banma(i,1) = real(ib)
!                  enddo
!                   if(myrank.eq.master) write(*,*)'Nobreakup',
!     &             bub2w_temp, D_eq_temp                    
!                else
!                  if(ib.eq.1)then
...
!                  if(myrank.eq.master)write(*,*)'Number of bubble'
!     &            ,i_num_bubbles
!
!                  endif ! for BubbleID = 1

               endif
         
!              endif  ! Only do this for BubbleID = 1 
