...
printf(
  ...
  (void*)&levlset.iLSet
    ...
    ((string)inp.GetValue("Flow Regime Transition") == "Enable")?  levlset.iFT=1:levlset.iFT=0;
    ...
    if((string)inp.GetValue("Coalescence Control") == "Active") {
      levlset.coalest = inp.GetValue("Number of Estimated Coalescence Events");
    }
    ((string)inp.GetValue("Use CFL at Bubble Interface") == "Yes")? levlset.buintcfl=1:levlset.buintcfl=0;
...
// ****************************************
    ...
    if ( (string)inp.GetValue("Bubbly boiling Mode") == "True")
        pcboiling.bubboil = 1.0; else pcboiling.bubboil = 0.0;
    if ((string)inp.GetValue("Bubble growth Mode") == "True")
        pcboiling.bubgrow = 1.0; else pcboiling.bubgrow = 0.0;
//    if ( pcboiling.solheat == 1){ 
//    vec = inp.GetValue("Specific Heat");
//	}
    if ( pcboiling.bubgrow == 1.0||pcboiling.solheat == 1.0){
    vec = inp.GetValue("Specific Heat");
    ...
    if( genpar.isurf > 0) {
      ...
      if((genpar.Bo >= 0) && (workfc.myrank==workfc.master)) {
         printf("\n Please update your solver.inp to use 'Inverse Surface Tension:' instead of 'Bond Number:'. \n ");
         cout << endl;
      }
      ...
      genpar.CoalInvSigma = inp.GetValue("Coalescence Contorl Inverse Surface Tension");
    }
...
//
    ...
    if ( (spebcvr.irscale = inp.GetValue("SPEBC Model Active")) >= 0 ){
     ...
     // Two-phase flow boiling heat tansfer
//     if ( (string)inp.GetValue("Bubbly boiling Mode") == "True")
//        pcboiling.bubboil = 1; else pcboiling.bubboil = 0;
 //    if ( (string)inp.GetValue("Bubble growth Mode") == "True")
//	pcboiling.bubgrow = 1; else pcboiling.bubgrow = 0;
     if ( pcboiling.solheat == 1.0){
        pcboiling.h_fg =  inp.GetValue("The latent heat");
        pcboiling.T_sat = inp.GetValue("The Saturation Temperature");
	pcboiling.epsilon_lst = inp.GetValue("Number of Elements Across Interface for boiling");
    }
      if (pcboiling.bubgrow == 1.0 || pcboiling.bubboil == 1.0){
       pcboiling.h_fg =  inp.GetValue("The latent heat");
       pcboiling.T_sat = inp.GetValue("The Saturation Temperature");
...
//       pcboiling.delt_T = inp.GetValue("The superheated rate");
       pcboiling.epsilon_lst = inp.GetValue("Number of Elements Across Interface for boiling");
      if (bubstudy.iBK == 1){
        pcboiling.breakup2w = inp.GetValue("The Bubble Departure Identification Region");
        pcboiling.breaksclr = inp.GetValue("The Limitation of Sclr1 for Seeding");
      }
    }
	...
	contactangle.CAramp = inp.GetValue("Contact Angle Force Ramp");
	contactangle.CAF_upper = inp.GetValue("Contact Angle Force Ramp Upper Limit");
	contactangle.CAF_interval = inp.GetValue("Contact Angle Force Ramp interval");
	contactangle.CA_startT = inp.GetValue("Contact Angle Force Ramp Activate Timestep");
	}
    // CAbRDIOVASCULAR MODELING PARAMETERS
    ...
    if ( nomodule.itvn ==1)
      ...
      nomodule.shvebct = inp.GetValue("Shift velocity");
      nomodule.iRBCT = inp.GetValue("Reading BCT");
