...
c      epsd=1.0d-12    ! this is distance SQUARED to save square root
      epsd=1.0d-12            ! this is distance SQUARED to save square root
       ...
       if (iRBCT.eq.0) then !read original bct.dat and generate multi bct files
               ...
               if(ibits(ibc(i),3,3) .eq.7) then
                  ...
                  if(dd.lt.epsd) then
  	               ...
  	               if (tvbcswitch.eq.0) then
                        ...
                        BCt(ic,j,1)=BCt(ic,j,1)+shvebct !Jinyong, Jan-2015:apply velocity shift
	               else
...
c
         ...
         enddo !ntv                 ! end of the loop over ntv
