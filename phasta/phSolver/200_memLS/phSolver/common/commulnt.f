...
c
     ...
     &          rDelIRecv, rDelISend, rDelWaitAll, TMRC
      integer   ilwork(nlwork), ierr, kdof, numtask, itkbeg,
     &          m, idl, itask, itag, iacc, iother, numseg, j,
     &          i, isgend, idof, jdl, lenseg, itemp, is, lfront,
     &          isgbeg, n
...
c
     ...
     &                     iother, itag, MPI_COMM_WORLD, req(m), ierr)
          ...
          if (code .eq. 'out') then
          !  write(*,*) 'commuInt req send'

            if(impistat.eq.1) iISend = iISend+1 !
            if(impistat.eq.1) rmpitmr = TMRC() !
            call MPI_ISEND(global(isgbeg, 1), 1, sevsegtype(itask,kdof), 
     &                     iother, itag, MPI_COMM_WORLD, req(m), ierr)
         !   write(*,*) 'commuInt req sent'            
      ...
      !write(*,*) 'commuInt waitall'
!      write(*,*) 'rank:',myrank, 'm:', m

!      write(*,*) 'stat is', stat
!      write(*,*) 'ierr is', ierr
      do i=1, m
       ! write(*,*) 'req(i) for m', m, 'is', req(i)
      enddo
      call MPI_WAITALL(m, req, stat, ierr)
!      write(*,*) 'commuInt waitalled'
c
