...
c Jinyong Feng, 2/8/2015. Print out the coordinates on the inflow surface boundary nodes:
...
c Otherwise, we are using the effective-viscosity wall-model and the 
...
c     
c.... calculate this element area-weighted normal vector
c     
                  e1 = x(ienb(2),:)-x(ienb(1),:)
                  e2 = x(ienb(3),:)-x(ienb(1),:)
                  elnrm(1) = e1(2)*e2(3)-e1(3)*e2(2)
...
c---------------------------------------------------------------------
c
c This routine counts up the total number of surface ID across
...
c count up the total number of surfID among all processes
         nsidt=0
         do j=1,numpe
            nsidt=nsidt+nsid(j)
         enddo
...
c  that has the largest projection onto the wall node normal vector
