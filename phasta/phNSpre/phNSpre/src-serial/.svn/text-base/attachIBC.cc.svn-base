...
// Need to save get and save the vertex here:
        if (vertex = VIter_next(vIter)) 
          { lvertex = vertex; 
          }
        else
      /* Flip sstop for everyone if tstop is true in a thread */
          { 
        cout << "sstop  = : "<< sstop << endl;
             sstop = 1;
//             #pragma omp flush(sstop)
             }
      }
       if(isActive((pEntity)lvertex))  {
            icount ++;
//        cout << "Assigning ICs, vertex: "<< icount <<";  thread: " << tn << endl;
            ic.eval(lvertex,q[Entity_getDOFnumber((pEntity)lvertex,0)]);
        }
...
/*    while (vertex = VIter_next(vIter))  {
	    ...
	    icount ++;
        cout << "Assigning ICs, vertex: "<<icount<<endl;	
