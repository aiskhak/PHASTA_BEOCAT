...
/*     if(NM_isAssemblyModel(pnModel)) {
	cout << "  in the parasolid NM_isAssembly loop  " << endl;
       pGAModel amodel = GAM_createFromNativeModel(pnModel, progress);
       NM_release(pnModel);
       model = GM_createFromAssemblyModel(amodel, NULL, progress);
       GM_release(amodel);
       pnModel = GM_nativeModel(model);
     }
     else
*/    
//     model = GM_createFromNativeModel(pnModel, progress);
// Here we need to "load" model file instead !

	model = GM_load(fname, pnModel, progress);   // CHECK fname !
