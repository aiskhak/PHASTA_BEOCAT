...
InitialCondition::InitialCondition(pGModel model)
{
  ...
  //Check if there is an external bubble information file
  //And print corresponding reminding message 
  FILE *fp;
  fp = fopen("bubbles.inp","rt");
  if(fp == NULL) {
    printf("\nThe File 'bubbles.inp' Not Found!\n");
    printf("NSpre will work without marker field\n");
    printf("\n");
  } else {
    printf("\nSuccessfully Found 'bubbles.inp'.\n");
    printf("Modified NSpre is working. \n");
    printf("Scalar 2 is used to store the marker field\n");
    printf("\n");
    fclose(fp);
  }
...
{
//  read coordinates of bubble centers and radius from external file
  FILE *ls_setup;
  double x[3];
  double tmp;				//Caution: need to be revised if we have more bubbles
  int nb = 0;                   //nb is for number of bubbles
  double  bi[1000][5];           //bi is for bubble information

//  int nsc;
  ls_setup=fopen("bubbles.inp","rt");
  if(ls_setup == NULL)
  {
   V_coord(vertex,x);
   if (info->nsd == 3) {
...
// Read from file the bubble distribution:
  } else {
           for (int i=0; i<1000; i++) {
            if(feof(ls_setup)) break;
            nb++;
            fscanf(ls_setup, "%lf %lf %lf %lf %lf\n", &bi[i][0], &bi[i][1], &bi[i][2], &bi[i][3], &bi[i][4]);
           }
	  // printf("Total number of bubbles read = %d \n", nb);
           ...
           if (info->nsd == 3) {
              ...
              if(attList[2]) q[4] = AttributeTensor0_evalDS((pAttributeTensor0) attList[2], x);
              if(attList[3]) {
                q[5] = 1e99; //MR: Extremely large number for initialization of the distance
                q[6] = 0.0;  //MR: bubble id if vertex inside a bubble, 0 otherwise
                for (int k=0; k<nb; k++) {        //kn is the index for neighbor bubbles
                    if(fabs(bi[k][1]-x[0])<100) {   //0.03142  determine where the neighborhood is
                        tmp = sqrt(pow((x[0]-bi[k][1]),2)+pow((x[1]-bi[k][2]),2)+pow((x[2]-bi[k][3]),2)) - bi[k][4];
                        if(tmp<q[5]) {
                            q[5] = tmp;
                            //q[6] = q[5];
                            if(q[5]<=0.0) q[6] = bi[k][0];
                        }
                    }
                }
               }
