...
namespace{
    ...
    StringStripper( const char*  istring ) {
        ...
        fname [ namelength ] = static_cast<char>(NULL);
        return fname;
    }

    inline int 
    cscompare( const char* s1, 
               const char* s2 ) {
...
//                    skip_size = atoi( token );
        ...
        if ( !FOUND ) {
            ...
            //Avoid the overwhelming error message
            return 1;
        }
        return 0;
    }	
...
writeheader_ (  int*  fileDescriptor,
    ...
    long long int size_of_nextblock = 
        ( binary_format ) ? type_size*( *ndataItems )+sizeof( char ) : *ndataItems ;
    
    fprintf( fileObject, "%s : < %lld > ", keyphrase, size_of_nextblock );
